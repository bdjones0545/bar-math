...
test("site.json host declares a public og:image origin even on *.vercel.app", () => {
  assert.equal(declaredPublicHost({ host: "bar-math.vercel.app" }), "bar-math.vercel.app");
  assert.equal(declaredPublicHost({ host: "Bar-Math.vercel.app:443" }), "bar-math.vercel.app");
  assert.equal(declaredPublicHost({ host: "" }), "");
  assert.equal(declaredPublicHost({}), "");
  const out = grokOgHeadTags({
    host: "bar-math-abc123-team.vercel.app",
    site: {
      title: "BAR MATH",
      card: "custom",
      banner: "/x-banner.jpg",
      host: "bar-math.vercel.app",
    },
    cwd: process.cwd(),
  }).join("\n");
  assert.match(out, /property="og:image" content="https:\/\/bar-math\.vercel\.app\/og\.jpg"/);
  assert.match(
    out,
    /property="x:game:image" content="https:\/\/bar-math\.vercel\.app\/x-banner\.jpg"/,
  );
});
