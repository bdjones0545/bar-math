    ...
    <div className="gym-shell lab-shell flex flex-col h-dvh overflow-hidden px-4 pb-[max(0.75rem,env(safe-area-inset-bottom))] pt-[max(0.75rem,env(safe-area-inset-top))]">
      <header className="flex items-center gap-2">
        ...
        <div className="flex-1 min-w-0 text-center">
          ...
          <h1 className="font-display tracking-[0.06em] text-lg leading-tight">POKE A MUSCLE</h1>
        </div>
...
function Play({
  ...
  function fail(id: MuscleId | null, outOfTime = false) {
  ...
  if (flash === "correct") {
    ...
    caption = {
      ...
      title: ended ? `${timedOutTitle} — ${muscle.name.toUpperCase()}` : muscle.name.toUpperCase(),
      subtitle: `${muscle.deep ? "Deep · " : ""}${q.cue}`,
      body: ended ? q.fact : named ? "Pick it from the list." : "Tap the highlighted region.",
    };
  } else if (flash === "wrong") {
    caption = {
      tone: "wrong",
  ...
  if (clock.phase === "idle") {
    return (
      <div className="mt-8 max-w-md mx-auto w-full text-center">
        <p className="font-display text-3xl tracking-[0.04em]">MUSCLE SPEED ROUND</p>
    ...
    <div className="mt-3 max-w-md mx-auto w-full flex flex-col flex-1 min-h-0">
      ...
      {clock.inIntro ? (
        <Countdown
          introMs={clock.intro}
          title="MUSCLE SPEED"
          subtitle="See the name. Tap the muscle."
        />
      ) : null}
    </div>
  );
